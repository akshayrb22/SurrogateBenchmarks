...
class GradientBoosting(ScikitBaseClass.ScikitBaseClass):
    def __init__(self, sp, encode, rng=1, **kwargs):
        ScikitBaseClass.ScikitBaseClass.__init__(self,
                                                 sp=sp,
                                                 encode=encode,
                                                 rng=rng,
                                                 **kwargs)
    ...
    def _random_search(self, random_iter, x, y):
        ...
        for idx, d in enumerate(param_list):
            ...
            train_x, test_x, train_y, test_y = train_test_split(
                x, y, test_size=0.5, random_state=self._rng)
            ...
            sys.stdout.flush()
        sys.stdout.write(
            "Using max_features %f, learning_rate: %f, max_depth: %d, min_samples_leaf: %d, "
    ...
    def train(self, x, y, param_names, **kwargs):
        ...
        # Do a random search
        max_features, learning_rate, max_depth, min_samples_leaf, n_estimators = self._random_search(
            random_iter=100, x=scaled_x, y=y)
        # Now train model
...
def test():
    ...
    sp = pcs_parser.read("../../tests/unittests/Testdata/nips2011.pcs")
    # Read data from csv
    header, data = read_csv("../../tests/unittests/Testdata/hpnnet_nocv_convex_all_fastrf_results.csv",
                            has_header=True,
                            num_header_rows=3)
