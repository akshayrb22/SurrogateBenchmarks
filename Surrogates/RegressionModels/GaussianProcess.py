...
from . import ScikitBaseClass
from Surrogates.DataExtraction.data_util import read_csv
from Surrogates.RegressionModels.GaussianProcess_src import gp_model, hyper_parameter_sampling
...
class GaussianProcess(ScikitBaseClass.ScikitBaseClass):
    ...
    def train(self,
        ...
        ls = numpy.ones(
            gp_model.getNumberOfParameters(cov_func_name, scaled_x.shape[1]))
        ...
        try:
            ...
            for h in range(len(_hyper_samples)):
                ...
                gp = gp_model.GPModel(X=scaled_x,
                                      y=y,
                                      mean=hyper[0],
                                      noise=hyper[1],
                                      amp2=hyper[2],
...
def test():
    ...
    header, data = read_csv(
        "../../tests/unittests/Testdata/hpnnet_nocv_convex_all_fastrf_results.csv",
        has_header=True,
        num_header_rows=3)
