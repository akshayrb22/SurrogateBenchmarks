...
def grad_dist2(ls, x1, x2=None):
    ...
    code = \
        ...
        """
    try:
        weave.inline(code, ['x1', 'x2', 'gX', 'ls', 'M', 'N', 'D'], type_converters=weave.converters.blitz,
                     compiler='gcc')
    except:
    ...
    def optimize_hypers(self, comp, vals):
        ...
        def grad_nlogprob(hypers):
            ...
            for dd in range(self.D):
                grad[dd + 2] = 1 * np.trace(np.dot(jacobian,
                                                   -amp2 * grad_corr[:, :, dd] * comp[:, dd][:, np.newaxis] / (
                                                       np.exp(ls[dd])))) * np.exp(ls[dd])
